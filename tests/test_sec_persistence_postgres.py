"""Live SEC shadow validation: real transactions, processes, restart, outage and collector parity."""
import json
...
from tests.test_sec_persistence_adapter import sample, NOW
from tests.test_sec_shadow_persistence import replay
...
def child_writer(url, schema, name, event, gate, results):
    ...
    try:
        ...
        def factory():
            ...
            return engine
        writer = shadow.SecShadowWriter(engine_factory=factory)
...
@live
class PostgreSQLSecAdapterTests(adapter.SecAdapterTests):
    setUp = foundation.PostgreSQLTests.setUp
    cleanup_schema = foundation.PostgreSQLTests.cleanup_schema


@live
class PostgreSQLSecOperationsTests(unittest.TestCase):
    setUp = foundation.PostgreSQLTests.setUp
    cleanup_schema = foundation.PostgreSQLTests.cleanup_schema
    count = adapter.SecAdapterTests.count
    reconcile = adapter.SecAdapterTests.reconcile

    def writer(self, **kwargs):
        writer = shadow.SecShadowWriter(engine_factory=lambda: self.engine, **kwargs)
    ...
    def test_repository_failure_rolls_back_without_collector_effect(self):
        ...
        with patch.object(EventRepository, "append_history", side_effect=RuntimeError("private")), \
             self.assertLogs("sec_shadow", level="WARNING"):
            writer = self.writer()
            enabled, submitted = replay(True, submit=writer.submit)
    ...
    def test_database_unavailable_at_start(self):
        ...
        with self.assertLogs("sec_shadow", level="WARNING") as logs:
            writer = shadow.SecShadowWriter(engine_factory=lambda: make_engine(missing))
            enabled, submitted = replay(True, submit=writer.submit)
    ...
    def test_restart_dedup_after_redis_expiry_through_worker(self):
        ...
        for label in ("repeat_after_ttl_expiry", "repeat_after_restart", "meta_8ka", "same_form_different_accession"):
            restarted.submit(sample(label))
        stats = restarted.shutdown()["stats"]
        self.assertEqual((stats["persisted"], stats["duplicate"], stats["failed"], stats["promotion_held"]), (4, 3, 0, 0))
        self.assertEqual(self.count(events), 3)
        for label in ("meta_8k", "meta_8ka", "same_form_different_accession"):
            self.assertEqual(self.reconcile(sample(label))["mismatches"], [])

    def test_worker_exception_rollback_then_recreation(self):
        event = sample("meta_10k")
        writer = self.writer()
        with patch.object(EventRepository, "append_history", side_effect=RuntimeError("private")):
            writer.submit(event)
    ...
    def concurrent_writers(self, first, second, *, lock_key):
        ...
        try:
            with self.engine.begin() as connection:
                connection.execute(events.insert().values(id=str(uuid4()), source_family="sec", identity_version="sec-v1",
    ...
    def test_two_processes_same_accession(self):
        event = sample()
        outcomes = self.concurrent_writers(event, event, lock_key=event["sec_fingerprint"])
    ...
    def test_two_processes_different_accessions(self):
        """Uncommitted inserts of the two new unique identities make both writers wait on the test transaction."""
        first, second = sample("meta_8k"), sample("same_form_different_accession")
        context = multiprocessing.get_context("spawn")
        gate, results = context.Barrier(3), context.Queue()
        prefix = "mias_sec_" + uuid4().hex[:12]
        url = require_test_database(DatabaseSettings(url=os.environ["TEST_DATABASE_URL"])).url.render_as_string(hide_password=False)
        ...
        try:
            with self.engine.begin() as connection:
                for event in (first, second):
                    connection.execute(events.insert().values(id=str(uuid4()), source_family="sec", identity_version="sec-v1",
                                                              event_key=event["sec_fingerprint"], first_seen_at=NOW, last_seen_at=NOW))
                for child in children: child.start()
                gate.wait(timeout=15)
                deadline, blocked = monotonic() + 4, 0
                while monotonic() < deadline:
    ...
    def test_real_container_down_midrun_and_recovery(self):
        ...
        try:
            ...
            self.assertEqual(enabled, baseline)  # Redis dedup and Telegram unchanged during the outage.
            ...
            recovered = sample("nvda_8k")
            writer.submit(recovered)
            await_stats(writer, "persisted", 3, timeout=15)
            self.assertFalse(self.reconcile(sample("meta_8k"))["event_found"])  # No automatic replay.
