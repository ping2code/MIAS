...
def run(workdir):
    ...
    evidence["finding_disclosure_promotion_after_expiry"] = disclosure_versions(url, first_resolved(processes, "C1-geo-lookup"))
...
def summarize(evidence):
    ...
    finding = evidence["finding_disclosure_promotion_after_expiry"]
    lines += ["", "### Finding (reported, not repaired): disclosure time promoted after Redis expiry", "",
    ...
    lines += [f"- `{v['published_at']}` current={v['current']}" for v in finding]
...
def render(evidence):
    ...
    lines = ["# Persistence Phase 2M: real-process staging report", "",
    ...
    sections = [("Environment", "environment"), ("Geopolitical live parity (staging vs control Redis)", "geo_live_parity"),
                ...
                ("FINDING: geopolitical disclosure time promoted after Redis expiry (not repaired)",
                 "finding_disclosure_promotion_after_expiry"),
