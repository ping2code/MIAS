"""Best-effort bounded Federal Reserve shadow writes; reuses the shared writer contract.

Never touches Redis: processed/delivered/claim keys remain owned by the collector.
"""
...
logger = get_logger("fed_shadow")


def persist_fed(engine, event, observed_at, *, make_current=True, report=False):
    """One atomic transaction: facts, evidence, then existing outcome snapshots."""
    adapted = adapt_fed(event, observed_at, make_current=make_current)
    with transaction(engine) as session:
        repo = EventRepository(session)
        row = repo.record(**adapted["record"], promotion_policy=fed_promotion)
...
def runtime_engine():
    return macro_shadow.runtime_engine(application_name="mias_fed_shadow")


_MESSAGES = {key: value.replace("Macro shadow", "Fed shadow") for key, value in macro_shadow._MESSAGES.items()}


class FedShadowWriter(macro_shadow.ShadowWriter):
    """Same queue, counters, logging bounds and shutdown semantics as macro."""
    logger = logger
    messages = _MESSAGES
    thread_name = "mias-fed-shadow"
    ...
    def _persist(self, engine, event, observed_at, make_current):
        # Module-level lookup keeps the Fed persist function patchable in tests.
        return persist_fed(engine, event, observed_at, make_current=make_current, report=True)


# Shared lifecycle; state stays in this module's namespace (see shadow_lifecycle).
_lifecycle = ShadowLifecycle(globals(), writer="FedShadowWriter", label="Fed")
submit_fed = _lifecycle.submit
