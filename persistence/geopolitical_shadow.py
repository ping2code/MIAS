...
def _submission_failure(reason):
    ...
    if warn:
        try:
            logger.warning("Geopolitical shadow submission unavailable; snapshot dropped")
        except Exception:
            pass
...
def submit_geopolitical(event, *, make_current=True):
    global _writer
    ...
    try:
        if _shutdown_requested:
            ...
            try:
                _writer = GeopoliticalShadowWriter()
            except Exception:
