"""Frozen initial foundation schema. Do not import application metadata here."""
...
def downgrade():
    ...
    if bind.dialect.name == "sqlite":
        bind.execute(events.update().values(current_version_id=None))
        event_provenance.drop(bind, checkfirst=False)
        event_versions.drop(bind, checkfirst=False)
        events.drop(bind, checkfirst=False)
    else:
